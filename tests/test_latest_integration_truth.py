...
def test_runtime_truth_surface_tracks_recent_landed_capabilities_without_promotion() -> None:
    ...
    assert "reference coefficients are not yet a complete compatible AxisSpace state" in stages[1]["implemented"]
    assert "AxisCoefficientSpace" in stages[1]["remaining"]
    assert "coefficientOperators/naturalRemainder" in stages[1]["remaining"]
    assert "referencePair radial coefficient functions" in stages[1]["remaining"]
