...
def _sum_model_precision(values) -> Decimal:
    with localcontext() as ctx:
        ctx.prec = DECIMAL_PRECISION
        return +sum(values, Decimal(0))


def _scale_model_precision(value: Decimal, power: int) -> Decimal:
    with localcontext() as ctx:
        ctx.prec = DECIMAL_PRECISION
        return +(Decimal(power) * value)
...
def test_complete_slow2_jet_is_exact_sum_of_four_landed_branches(slow2_state) -> None:
    ...
    for name in FIELDS:
        assert getattr(actual, name) == _sum_model_precision(
            getattr(jet, name) for jet in constituent_jets
        )
...
def test_complete_slow2_retains_signed_log_pressure_scales(slow2_state) -> None:
    ...
    for numerator, logged in zip(
        ...
        if numerator != 0:
            assert logged.log_scale == _scale_model_precision(selected.amplitude_log, 2)
    ...
    if numerator != 0:
        assert square.log_scale == _scale_model_precision(selected.amplitude_log, 4)
