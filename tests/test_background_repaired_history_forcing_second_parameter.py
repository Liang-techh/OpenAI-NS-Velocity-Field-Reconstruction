...
@pytest.mark.parametrize(
    "xi",
    [0.0, math.sqrt(0.5 * 2.17**2), math.sqrt(0.5 * 4.17**2)],
)
def test_second_eta_forcing_matches_first_parameter_finite_difference(xi):
    hierarchy = _hierarchy()
    order = 2
    eta = 0.23
    eps = 2.0e-5

    actual = hierarchy_owned_positive_axis_forcing_second_parameter_jet(
        hierarchy,
        order,
        xi,
        eta,
    ...
    plus = hierarchy_owned_positive_axis_forcing_parameter_jet(
        hierarchy,
        order,
        xi,
        eta + eps,
    )
    minus = hierarchy_owned_positive_axis_forcing_parameter_jet(
        hierarchy,
        order,
        xi,
        eta - eps,
    ...
    assert np.array_equal(actual.value, first.value)
    assert np.array_equal(actual.parameter, first.parameter)
    finite_difference = (plus.parameter - minus.parameter) / (2.0 * eps)
    assert actual.parameter2 == pytest.approx(
        finite_difference,
        rel=4.0e-4,
        abs=4.0e-6,
    )
    assert np.array_equal(actual.parameter2[:3], np.zeros(3))


def test_second_eta_forcing_requires_phi_fourth_hierarchy():
...
def test_second_eta_forcing_fails_closed_without_leading_phi_fourth_ownership():
