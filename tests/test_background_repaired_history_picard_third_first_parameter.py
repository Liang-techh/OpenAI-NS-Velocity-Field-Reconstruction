...
def _source0():
    return Section5SixthMixedCoefficientJetSource(
        ...
        provenance="analytic third-Picard regression fixture; not paper coefficient data",
...
def _rhs(hierarchy, order, s, eta, derivative):
    ...
    if derivative == 0:
        return matrices.A0 @ second.value + matrices.A1 @ second.parameter
    if derivative == 1:
        return (
...
def test_third_picard_eta1_value_is_exact_K_of_second_picard_term():
    hierarchy = _hierarchy()
    order = 1
    xi = 0.16
    eta = 0.12
    actual = hierarchy_owned_third_picard_first_parameter_jet(
        hierarchy,
        order,
        xi,
        eta,
...
def test_third_picard_eta1_uses_exact_first_leibniz_row():
    hierarchy = _hierarchy()
    order = 1
    xi = 0.14
    eta = -0.11
    ...
    expected = singular_inverse_eq_5_7(
        xi,
        eta,
        lambda s, e: _rhs(hierarchy, order, s, e, 1),
        quadrature_points=hierarchy.quadrature_points,
    )
    np.testing.assert_allclose(actual.parameter, expected, rtol=3e-13, atol=3e-13)


def test_third_picard_eta1_axis_rows_are_exactly_zero_and_read_only():
    actual = hierarchy_owned_third_picard_first_parameter_jet(
        _hierarchy(),
        1,
        0.0,
    ...
    for row in (actual.value, actual.parameter):
        np.testing.assert_array_equal(row, np.zeros(6))
        assert not row.flags.writeable


def test_third_picard_eta1_fails_closed_on_wrong_hierarchy_type():
    with pytest.raises(TypeError, match="Section5LowerHistorySixthMixedHierarchy"):
        hierarchy_owned_third_picard_first_parameter_jet(object(), 1, 0.0, 0.0)
