...
@pytest.mark.parametrize("X", [0.0, 0.5 * 2.17**2, 0.5 * 4.17**2])
def test_second_eta_lower_source_matches_first_parameter_finite_difference(X):
    hierarchy = _hierarchy()
    order = 2
    eta = 0.23
    eps = 2.0e-5

    actual = hierarchy_owned_lower_source_second_parameter_jet(
        hierarchy,
        order,
        X,
        eta,
    ...
    plus = hierarchy_owned_lower_source_parameter_jet(
        hierarchy,
        order,
        X,
        eta + eps,
    )
    minus = hierarchy_owned_lower_source_parameter_jet(
        hierarchy,
        order,
        X,
        eta - eps,
    ...
    for field in ("angular", "axial", "pressure_product", "omega_quotient"):
        finite_difference = (
            getattr(plus.parameter, field) - getattr(minus.parameter, field)
        ) / (2.0 * eps)
        assert getattr(actual.parameter2, field) == pytest.approx(
            finite_difference,
            rel=3.0e-4,
            abs=3.0e-6,
        )
...
def test_second_eta_source_requires_phi_fourth_hierarchy():
    with pytest.raises(TypeError, match="Section5LowerHistoryPhiFourthMixedHierarchy"):
        hierarchy_owned_lower_source_second_parameter_jet(object(), 1, 0.5, 0.1)


def test_second_eta_source_fails_closed_without_leading_phi_fourth_ownership():
