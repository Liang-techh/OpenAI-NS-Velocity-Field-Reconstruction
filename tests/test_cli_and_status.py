...
def test_runtime_status_tracks_current_formal_structure_without_promotion():
    ...
    assert 'axisPressure' in stages[1]['remaining']
    assert 'finalAngular/clockWeight/axisPressure' not in stages[1]['remaining']
    ...
    assert 'Lemma 6.1' in stages[3]['remaining']
    assert 'Construct the actual squared slow partitions' not in stages[3]['remaining']
