...
def test_runtime_status_tracks_current_formal_structure_without_promotion():
    ...
    assert 'Eq. (5.15)' in stages[2]['remaining']
    assert 'true uniform compactness bounds C[j,m]' in stages[2]['remaining']
    assert 'implement Lemma 5.2' not in stages[2]['remaining']
    ...
    assert 'SlotColoring.Adj' in stages[3]['remaining']
    assert 'full Lemma 6.1 cross-label separation' in stages[3]['remaining']
    assert 'Construct the common auxiliary rectangle centers/radius' not in stages[3]['remaining']
    ...
    assert 'actual localized NS residual' in stages[7]['remaining']
    ...
    assert 'construct the official derivative-majorant/local-scale bounds' not in stages[7]['remaining']
