...
def _base_phi_fifth_mixed_jet(X, eta):
    ...
    rows = [e * _poly_derivative(coeff, eta, derivative) for derivative in range(6)]
...
def _leading_strong_source():
    return Section5PhiFifthMixedCoefficientJetSource(
        order=0,
        phi_second_jet_provider=lambda X, eta: _base_phi_fifth_mixed_jet(
            X, eta
        ).fourth().third().second(),
        ...
        provenance=(
            "analytic phi-fifth/U-fifth leading test fixture; Issue #1 remains upstream"
        ),
        axial_fourth_mixed_jet_provider=lambda X, eta: _base_u_fifth_mixed_jet(
            X, eta
        ).fourth(),
        phi_third_mixed_jet_provider=lambda X, eta: _base_phi_fifth_mixed_jet(
            X, eta
        ).fourth().third(),
        axial_fifth_mixed_jet_provider=_base_u_fifth_mixed_jet,
        phi_fourth_mixed_jet_provider=lambda X, eta: _base_phi_fifth_mixed_jet(
            X, eta
        ).fourth(),
...
def _hierarchy():
    repaired = (
        Section5PhiFifthMixedCoefficientJetSource.from_lemma52_repair_full_strong_mixed(
            1,
            C=C,
...
def test_hierarchy_owns_repaired_phi_fifth_and_exact_lower_projections():
    functional = _functional_repair()
    hierarchy = _hierarchy()
    X = 0.5 * 4.17**2
    eta = -0.23

    independent = Lemma52RepairedPhiFifthMixedJetAdapter(
        functional,
        C,
        _base_phi_fifth_mixed_jet,
    )
    expected = independent.phi_fifth_mixed_jet(X, eta)
    actual = hierarchy.phi_fifth_mixed_jet(1, X, eta)

    assert actual == expected
    assert actual.fourth() == hierarchy.phi_fourth_mixed_jet(1, X, eta)
    assert actual.fourth().third() == hierarchy.phi_third_mixed_jet(1, X, eta)
    assert (
        actual.fourth().third().second()
        == hierarchy.phi_second_jet(1, X, eta)
    )
...
def test_phi_fifth_query_fails_closed_for_previous_phi_fourth_source():
    weak = Section5PhiFourthMixedCoefficientJetSource(
        order=0,
        phi_second_jet_provider=lambda X, eta: _base_phi_fifth_mixed_jet(
            X, eta
        ).fourth().third().second(),
        ...
        provenance="previous strong source without fifth-mixed phi ownership",
        axial_fourth_mixed_jet_provider=lambda X, eta: _base_u_fifth_mixed_jet(
            X, eta
        ).fourth(),
        phi_third_mixed_jet_provider=lambda X, eta: _base_phi_fifth_mixed_jet(
    ...
    hierarchy = Section5LowerHistoryPhiFifthMixedHierarchy(H, C, (weak,))

    with pytest.raises(ValueError, match="fifth-mixed phi"):
        hierarchy.phi_fifth_mixed_jet(0, 0.0, 0.1)


def test_strong_hierarchy_rejects_incoherent_fifth_to_fourth_projection():
    def bad_fourth(X, eta):
        fourth = _base_phi_fifth_mixed_jet(X, eta).fourth()
        return replace(fourth, parameter4=fourth.parameter4 + 1.0)

    bad = Section5PhiFifthMixedCoefficientJetSource(
        order=0,
        phi_second_jet_provider=lambda X, eta: _base_phi_fifth_mixed_jet(
            X, eta
        ).fourth().third().second(),
        ...
        provenance="deliberately incoherent phi fifth/fourth test fixture",
        axial_fourth_mixed_jet_provider=lambda X, eta: _base_u_fifth_mixed_jet(
            X, eta
        ).fourth(),
        phi_third_mixed_jet_provider=lambda X, eta: _base_phi_fifth_mixed_jet(
        ...
        axial_fifth_mixed_jet_provider=_base_u_fifth_mixed_jet,
        phi_fourth_mixed_jet_provider=bad_fourth,
        phi_fifth_mixed_jet_provider=_base_phi_fifth_mixed_jet,
    )
    hierarchy = Section5LowerHistoryPhiFifthMixedHierarchy(H, C, (bad,))

    with pytest.raises(ValueError, match="incoherent"):
        hierarchy.phi_fifth_mixed_jet(0, 0.5 * 4.17**2, 0.2)
