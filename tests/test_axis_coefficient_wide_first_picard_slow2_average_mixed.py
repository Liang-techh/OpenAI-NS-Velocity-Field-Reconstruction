...
def branch_state():
    ...
    return wide_first_picard_slow2_average_mixed_state(x1)
...
def _literal_mixed(x1, n: int, m: int, eta: float):
    ...
    with localcontext() as ctx:
        ...
        for i in range(source_degree + 1):
            ...
            for k in range(m + 1):
                ...
                left_o = _average_ordinary(x1, i, k + 1, eta)
                right_o = _u_ordinary(x1, j, l, eta)
                left_p = _average_pressure(x1, i, k + 1, eta)
...
def _literal_full(branch_state, n: int, m: int, eta: float):
    if n == 0:
    ...
    with localcontext() as ctx:
        ...
        for k in range(m + 1):
            l = m - k
            weight = Decimal(math.comb(m, k)) * Decimal.from_float(data.d.jet(0, k, eta))
            mixed_o, mixed_p, mixed_s = _literal_mixed(x1, n, l, eta)
            for power, value in enumerate(mixed_o):
                ordinary[power] += weight * value
            for power in range(1, 4):
                pressure_linear[power] += weight * mixed_p[power]
            pressure_square += weight * mixed_s
...
def test_branch_is_bound_to_genuine_first_picard_state(branch_state) -> None:
    assert isinstance(branch_state, ActualScheduleWideFirstPicardSlow2AverageMixedState)
    ...
    assert branch_state.slow2_average_mixed_branch_materialized is True
...
def test_ordinary_reference_crosschecks_landed_operator_composition(branch_state) -> None:
    ...
    expected_state = operators.product(
        data.d,
        operators.mixed1(operators.average(reference_u), reference_u),
    )
...
def test_all_scale_numerators_follow_literal_mixed_then_product_formula(branch_state) -> None:
    n, m, eta = 4, 2, 0.03
    actual = branch_state.jet(n, m, eta)
    literal = _literal_full(branch_state, n, m, eta)

    assert isinstance(actual, MixedScaleFirstPicardSlow2AverageMixedCoefficientJet)
    for name in FIELDS:
...
def test_row_zero_is_exact_zero_without_skipping_upstream_state(branch_state) -> None:
...
def test_nonzero_pressure_terms_keep_a2_and_a4_scales(branch_state) -> None:
    ...
    with localcontext() as ctx:
        ...
        for power, (numerator, logged) in enumerate(
            zip(numerators, actual.pressure_linear_terms_log()), start=1
        ):
            if numerator == 0:
...
def test_branch_rejects_invalid_indices_window_and_surrogate_state(branch_state) -> None:
    ...
    with pytest.raises(TypeError, match="x1 must be ActualScheduleWideFirstPicardState"):
        wide_first_picard_slow2_average_mixed_state(object())
...
def test_branch_provenance_is_machine_readable_and_fail_closed() -> None:
    ...
    assert "mixed1" in layer["capability"]
