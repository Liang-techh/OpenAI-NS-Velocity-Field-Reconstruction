...
def solve_q_from_tau(z: float, tau: float, h: float, *,
    ...
    Set q=scale*exp(y), scale=max(tau, |z|^(1/D)), y>=0. The
    monotone equation is 1-exp(log_gamma-(1-2h)*y)-exp(log_a-y)=0.
    ``expm1`` avoids cancellation at the endpoint |eta|=1. An overflow
    or failure to converge is reported, never silently converted to a value.
    """
    ...
    if residual(0.0) == 0.0:  # correction smaller than representable precision
        return scale
    ...
    for _ in range(2048):
        ...
        if log_scale + hi > math.log(sys.float_info.max):
            # Do not give up merely because a tentative upper bracket overflowed.
    ...
    for _ in range(max_iter):
        ...
        if hi - lo <= math.log1p(rtol):
            y = (lo + hi) * 0.5
            # Multiplication preserves exact tau when it is the dominant scale.
            q = scale * math.exp(y)
...
def similarity_coordinates_from_tau(r: float, z: float, tau: float,
    ...
    # A floating-point endpoint eta=+-1 can coexist with small positive d.
    d = tau / q
    L = (1.0 - 2.0 * h) + 2.0 * h * d
...
def coordinate_identity_error(r: float, z: float, t: float, h: float) -> tuple[float, float]:
    """Original defining-identity errors, retaining a non-tautological tau check."""
