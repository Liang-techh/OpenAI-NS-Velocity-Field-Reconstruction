"""CLI: audit reconstruction status or generate explicitly labelled diagnostics."""
from __future__ import annotations
import argparse
import json
from pathlib import Path
import sys
from .provenance import status_report, require_complete_reconstruction, IncompleteReconstructionError
...
def main(argv=None) -> int:
    ...
    audit.add_argument('--require-paper-exact',action='store_true',help='exit 2 if any required stage is incomplete')
    demo=sub.add_parser('demo',help='export toy samples and independent numerical diagnostics')
    demo.add_argument('--output',type=Path,default=Path('results'))
    demo.add_argument('--require-paper-exact',action='store_true',help='refuse incomplete reconstruction before writing data')
    ...
    try:
        ...
        print(json.dumps({"report":str(args.output/'report.json'),
                          ...
                          "all_diagnostic_checks_passed":report['all_diagnostic_checks_passed']},indent=2))
