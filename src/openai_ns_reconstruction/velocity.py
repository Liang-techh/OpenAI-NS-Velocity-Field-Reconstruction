...
import math
import numpy as np
from .coordinates import SimilarityPoint, _finite, similarity_coordinates, similarity_coordinates_from_tau
...
def leading_velocity_from_tau(r: float, z: float, tau: float,
                              profile: LeadingProfile, *, h: float = 0.005,
                              quadrature_points: int = 32) -> CylindricalVelocity:
    """Compatibility alias for the direct-tau cylindrical evaluator."""
    return leading_velocity_cylindrical_from_tau(
        r, z, tau, profile, h=h, quadrature_points=quadrature_points
    )
...
def leading_pressure_cartesian(x: float, y: float, z: float, t: float,
                               profile: LeadingProfile, *, h: float = 0.005) -> float:
    """Compatibility alias retained for the newer main-branch diagnostics."""
    return leading_pressure(x, y, z, t, profile, h=h)
