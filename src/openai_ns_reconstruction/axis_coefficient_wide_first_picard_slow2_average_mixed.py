"""Pinned ``d * mixed1(average(u1), u1)`` branch of ``slow2`` at first Picard state.

The pinned ``AxisContraction.naturalRemainder.slow2`` contains

    product d (mixed1 (average u) u)

with ``mixed1 = inverseMixed 1``.  At the genuine first Picard state,

    u1 = U0 + U1/Lambda + U2/Lambda^2 + a^2 P/Lambda,

this module propagates the already-landed theorem-selected split state through
``average``, pinned ``mixed1``, and multiplication by the actual AxisData field
``d = 1 - eta^2`` without collapsing scales.  The result remains separated into
ordinary powers ``Lambda^0..Lambda^-4``, pressure-linear ``a^2`` powers
``Lambda^-1..Lambda^-3``, and the pressure-square ``a^4 Lambda^-2`` term.

For output row ``n>=1``, pinned ``mixed1`` reads ``i+j=n-1`` with factor
``j/n^2`` and places one additional eta derivative on ``average(u1)``.  The
outer multiplication by ``d`` uses the exact eta-Leibniz rule.  The actual
AxisData ``d`` is radial-degree zero, so no invented radial cutoff is required.
No caller-supplied d, Lambda, amplitude, pressure, coefficient table, or
surrogate state is accepted.

This is one mixed-scale ``slow2(x1)`` branch only.  It is not complete
``slow2(x1)``, ``naturalRemainder(x1)``, ``x2``, a fixed-point certificate,
global weighted ``AxisSpace`` membership, or paper-exact velocity.
"""
...
class MixedScaleFirstPicardSlow2AverageMixedCoefficientJet:
    """One coefficient jet of ``d * mixed1(average(u1), u1)``."""
...
@dataclass(frozen=True)
class ActualScheduleWideFirstPicardSlow2AverageMixedState:
    """Pinned ``d*mixed1(average(u1),u1)`` branch on theorem-selected ``x1``."""
    ...
    def _mixed_raw(
        ...
        if n == 0:
            return ordinary, pressure_linear, pressure_square
        ...
        with localcontext() as ctx:
            ...
            for i in range(source_degree + 1):
                ...
                for k in range(m + 1):
                    ...
                    left_ordinary = self._average_ordinary(i, k + 1, eta)
                    ...
                    left_pressure = self._average_pressure_numerator(i, k + 1, eta)
            ...
            return (
                [+value for value in ordinary],
                [+value for value in pressure_linear],
                +pressure_square,
            )
    ...
    def _zero(
        *,
        ...
        return MixedScaleFirstPicardSlow2AverageMixedCoefficientJet(
    ...
    def jet(
        ...
        """Evaluate pinned ``d * mixed1(average(u1), u1)`` at one coefficient jet."""

        n = _index(n, "n")
        m = _index(m, "m")
        eta = _eta_in_window(eta)

        # Keep the genuine theorem-data chain live even on the exact zero row.
        ...
        # CoefficientFamily.axisData proves d is radial-degree zero.  Therefore
        # product(d, mixed1(...))[n,m] has only the eta-Leibniz convolution.
        with localcontext() as ctx:
            ctx.prec = _DECIMAL_PRECISION
            for k in range(m + 1):
                l = m - k
                d_jet = Decimal.from_float(data.d.jet(0, k, eta))
                weight = Decimal(math.comb(m, k)) * d_jet
                mixed_ordinary, mixed_pressure, mixed_square = self._mixed_raw(n, l, eta)
                for power, value in enumerate(mixed_ordinary):
                    ordinary[power] += weight * value
                for power in range(1, 4):
                    pressure_linear[power] += weight * mixed_pressure[power]
                pressure_square += weight * mixed_square
...
def wide_first_picard_slow2_average_mixed_state(
    x1: ActualScheduleWideFirstPicardState,
) -> ActualScheduleWideFirstPicardSlow2AverageMixedState:
    """Lift the pinned average/u ``mixed1`` branch without theorem-scale collapse."""

    return ActualScheduleWideFirstPicardSlow2AverageMixedState(x1=x1)
