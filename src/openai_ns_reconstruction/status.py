...
STAGES = [
    ...
    {"id": 1, "name": "leading profile", "status": "formal-structure",
     "implemented": (
         ...
         "to conservative remainderBound/remainderLip values, and the canonical analytic-input norm adapter: from an "
         "independently certified common complex neighborhood radius rho>0 and common field-value bound B it uses "
         "epsilon=rho/2, radiusLoss(1/2)=12, fixed-field norm <=12B, normalized amplitude M=12, ||chi||<=12B, "
         "K<=30720B, and a conservative complete factorial-series enclosure for the naturalResolvent norm."
     ),
     "remaining": (
         "Materialize and certify an actual admissible common analytic radius rho and common complex-field sup bound B "
         "for the schedule-pressure coefficient family, plus the true complex compact-set realPartSup; feed those genuine "
         "witnesses through the landed analytic-input/remainder/Lambda-C chain to materialize the coefficient-space "
    ...
    {"id": 2, "name": "all-order background", "status": "formal-structure",
     "implemented": (
         ...
         "doubling envelope, an exact finite-prefix cutoff support certificate recording plateau, the unique possible "
         "transition order, forced-zero tail within the constructed prefix, and stable truncation once that zero tail is "
         "reached, plus a fixed-prefix tail-order arithmetic gate recording the Lean bounds 2^-J q^(h(J+1)-m) and the "
         "physical exponent h(J+1)+b-2M, solving exactly for the minimal J needed for a requested finite decay order and "
         "failing closed when the constructed finite scale schedule is too short."
     ),
     "remaining": (
         ...
         "cutoff sequence; then prove theorem-level local finiteness, the finite coefficient identities that cancel the NS "
         "residual order by order, and Proposition 5.3/all-jets-flat residual decay rather than extrapolating from finite-prefix "
         "support or tail-exponent arithmetic."
     )},
    {"id": 3, "name": "dyadic charts and transported phases", "status": "formal-structure",
     ...
     "remaining": (
         ...
         "then feed those genuine hypotheses through the landed uniform normal and frame/damping envelopes. Continue only "
         "after that to the stress-cone, amplitude/curl wave, mean-correction and residual-improvement stages."
     )},
    ...
    {"id": 7, "name": "final compact field and force", "status": "formal-structure",
     "implemented": (
         ...
         "for t>=T, and composes the normal-jet contraction with the existing locally finite Borel future branch, plus an "
         "exact support-cylinder implication from SpatialLocalization: support lies in r^2<=1/16, |z|<=1/4 (volume pi/32), "
         "so an independently certified fixed-time |u|<=M bound yields E(t)<=pi M^2/64 without inferring M from samples."
     ),
     "remaining": (
         ...
         "establish the paper's uniform bounded kinetic-energy conclusion along the actual blow-up limit (the landed "
         "fixed-time support/sup-norm implication is insufficient), and connect completed upstream local-field inputs."
     )},
