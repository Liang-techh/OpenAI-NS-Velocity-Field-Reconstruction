...
STAGES = [
    ...
    {"id": 1, "name": "leading profile", "status": "formal-structure",
     "implemented": (
         ...
         "grid maxima or fitted complex data."
     ),
     "remaining": (
         "Feed the actual-schedule rho/B/realPartSup certificate through the landed analytic-input/remainder/Lambda-C chain, "
         "control the very large conservative scales without weakening the contraction hypotheses, and materialize the "
         "coefficient-space fixed-point phi/u/average/pressure fields. Connect those genuine fields to "
         "NaturalProfileAssembly, then verify the paper's support, moments, matching and cone conditions. The current "
    ...
    {"id": 2, "name": "all-order background", "status": "formal-structure",
     "implemented": (
         ...
         "forced-zero tail within the constructed prefix and stable truncation once that zero tail is reached, plus a "
         ...
         "the constructed finite scale schedule is too short."
     ),
     "remaining": (
         ...
         "theorem-level local finiteness, finite coefficient identities cancelling the NS residual order by order, and "
         "Proposition 5.3/all-jets-flat residual decay."
     )},
    ...
    {"id": 4, "name": "oscillatory stress realization", "status": "formal-structure",
     "implemented": (
         ...
         "sqrt(epsilon)*mask amplitude localization."
     ),
     "remaining": (
         "Prove the actual pulse-integrated covariance columns are a controlled perturbation of the reference matrix, "
         "deduce determinant/inverse stability and persistence of positive amplitudes for the real T_{0,*} data, then "
         "solve the amplitude transport/ODE and realize the oscillatory corrections by supported divergence-free curls."
     )},
    ...
    {"id": 7, "name": "final compact field and force", "status": "formal-structure",
     "implemented": (
         ...
         "with supplied-bound 2^-j derivative-tail certificates, a value-level CandidateFromLimits trace/glue bridge "
         "that preserves the supplied closed-past residual for t<T, uses only the validated degree-zero endpoint tensor "
         "for t>=T, and composes the normal-jet contraction with the existing locally finite Borel future branch, an exact "
         "support-cylinder implication from SpatialLocalization: support lies in r^2<=1/16, |z|<=1/4 (volume pi/32), so "
         "an independently certified fixed-time |u|<=M bound yields E(t)<=pi M^2/64 without inferring M from samples, "
         "and a fail-closed late-origin preservation certificate for 3/4<=t<1 showing the official spatial plateau gives "
         "c=1 and grad(c)=0 at the origin while the time switch is exactly one, so final localization preserves any "
         "upstream-certified origin curl value without sampling a blow-up sequence."
     ),
     "remaining": (
         ...
         "normal jets arise from those actual limits so the resulting global force is smooth through t=1. Independently "
         "establish the paper's uniform bounded kinetic-energy conclusion along the actual blow-up limit, connect completed "
         "upstream local-field inputs, and prove the required upstream ||curl A(t,0)||->infinity premise; the landed "
         "late-origin adapter preserves that premise but does not establish it."
     )},
...
def construction_status() -> dict:
    return {
        ...
        "limitations": [
            ...
            "The Section 7 stress-cone module is only the exact reference algebra; actual pulse-covariance perturbation and wave realization are still missing.",
        ],
