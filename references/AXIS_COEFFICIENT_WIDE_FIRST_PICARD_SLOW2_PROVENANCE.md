...
drop nonzero theorem-scale terms by underflow.

## Verification boundary

Regression verifies that the complete jet is the exact componentwise sum of
all four landed branch jets, including the exact row-zero result, shared
`x1/Lambda/epsilon/amplitude` identity, signed-log scale retention, invalid
index/window rejection, and surrogate-state rejection.
