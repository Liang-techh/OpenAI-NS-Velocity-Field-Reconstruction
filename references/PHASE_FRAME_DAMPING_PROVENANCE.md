...
- `base_phase_constant` ↔ family-level `BasePhaseGeometry.phaseConstant`, namely `normalConstant M = 8 * PhaseEstimates.phaseConstant (2*M)`.
...
- `coordinate_constant` and `damping_constant` ↔ the named constants used by `FamilyData.coordinate_errors`, `FamilyData.damping_error`, and `FamilyData.coefficientControl` after the specialization `A=3M`, `delta=phaseConstant(M)/S`, `eta=16 M^2/S`, `b=normalLower(M,u)`.
...
Independent checks in `tests/test_phase_frame_bounds.py` recompute the theorem arithmetic directly, verify the large-band specialization reduces exactly to `coordinateConstant/S` and `dampingConstant/S`, and exercise fail-closed paths for every scalar smallness/boundedness hypothesis.
