...
`3cc438f29524acb068e40bf3a99a8293c9222628`, after the landed coefficient-wide
first-Picard `slow2(x1)` composition from PR #293, the Section 8 exact compact-support
geometry from PR #305, the Section 8 exact five-row algebra from PR #313, and the
Section 9 exact residual-improvement arithmetic from PR #309. Those landed results
are preserved as upstream/parallel work; this replay does not modify or replace
them. The Stage-1 upstream state remains partial: axial all-order reference jets,
the certified infinite weighted `AxisCoefficientSpace.ofJetFamily` bound, a
complete typed reference / fixed-point state, genuine `naturalRemainder`, `x2`, and
the materialized fixed point are still absent from this baseline. This increment
consumes none of those missing objects and does not duplicate the coefficient backend
lane.
...
The production and regression payload is replayed byte-for-byte unchanged from
the prior Stage-1 profile-assembly contract PR; only the integration baseline/context
is refreshed to current `main`. Fresh repository Actions on this exact replay head
