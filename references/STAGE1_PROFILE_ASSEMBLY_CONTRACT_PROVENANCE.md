...
`95ba525c8aadab0ae4b05610b2042c9a02fec234`, after the landed coefficient-wide
first-Picard `slow2(x1)` composition from PR #293 and the unrelated Section 8 exact
compact-support geometry from PR #305. Those landed results are preserved as
upstream/parallel work; this replay does not modify or replace them. The Stage-1
upstream state remains partial: axial all-order reference jets, the certified
infinite weighted `AxisCoefficientSpace.ofJetFamily` bound, a complete typed
reference / fixed-point state, genuine `naturalRemainder`, `x2`, and the
