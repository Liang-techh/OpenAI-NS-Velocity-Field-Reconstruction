...
`Section9ResidualEndpointSourceBinding` then requires exact agreement with an already-admitted `Section9EndpointLadderBridgeRecord` in stage, spatial window, derivative count, and every ordered majorant provenance string. The existing finite ladder must remain formally ready. Sampled/fitted source evidence is rejected and no source identity is inferred from numerical values.
...
The purpose here is narrower: make the next genuine bridge from the actual Section 9 residual estimates into the `t=1` endpoint extension fail closed unless the majorants and the residual jets are demonstrably about the **same residual revision**.
